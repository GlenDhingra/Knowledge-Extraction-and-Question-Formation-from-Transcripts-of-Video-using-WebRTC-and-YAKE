...
mongoose
  .connect(
    "mongodb+srv://ahmed:Ahmed123@nodeexpressprojects.isklh.mongodb.net/BE-Project?retryWrites=true&w=majority",
    {
...
app.post("/login", (req, res) => {
  ...
  Users.findOne(
    { email: email, user_password: sha256(password) },
    (err, data) => {
      if (err) {
        console.log(err);
        res.render("login", {
...
app.get("/", redirectLogin, (req, res) => {
  ...
  Rooms.find(
    { members_attended: user.email, active: false },
    [
      "room_id",
      "created_by_user",
      "members_attended",
    ...
    { limit: 20, sort: { start_date_time: -1 } },
    (err, data) => {
      if (err) {
        console.log(err);
